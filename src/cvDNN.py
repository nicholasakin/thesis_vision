...
parser.add_argument('--dir0', '-d0', type=str, help='Path to the image dir to crop. USED FOR FACIAL CROPPING') 
parser.add_argument('--image1', '-i1', type=str, help='Path to the input image1. Omit for detecting on default camera.')
parser.add_argument('--image2', '-i2', type=str, help='Path to the input image2. When image1 and image2 parameters given then the program try to find a face on both images and runs face recognition algorithm.')
parser.add_argument('--video', '-v', type=str, help='Path to the input video.')
parser.add_argument('--scale', '-sc', type=float, default=0.9, help='Scale factor used to resize input video frames.')
parser.add_argument('--face_detection_model', '-fd', type=str, default='models/yunet_032022.onnx', help='Path to the face detection model. Download the model at https://github.com/opencv/opencv_zoo/tree/master/models/face_detection_yunet')
...
parser.add_argument('--save', '-s', type=str2bool, default=True, help='Set true to save results. This flag is invalid when using camera.')
args = parser.parse_args()

#args.video = "src/channing_tatum_short.mp4"
args.dir0 = "data/nicholas_akin"

def visualize(input, faces, fps, thickness=2):
    if faces[1] is not None:
        for idx, face in enumerate(faces[1]):
            print('Face {}, top-left coordinates: ({:.0f}, {:.0f}), box width: {:.0f}, box height {:.0f}, score: {:.2f}'
    ...
    cv.putText(input, 'FPS: {:.2f}'.format(fps), (1, 16), cv.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2) #show fps

def crop(input, faces):
    if faces[1] is not None:
        for idx, face in enumerate(faces[1]):

            coords = face[:-1].astype(np.int32) #removes last elem
            # print("coords[0]: ", coords[0])
            # print("coords[1]: ", coords[1])
            # print("coords[2]: ", coords[0]+coords[2])
            # print("coords[3]: ", coords[1]+coords[3])

            input = input[coords[1]:coords[1]+coords[3], coords[0]:coords[0]+coords[2]]
            return input
...
if __name__ == '__main__':
    ...
    #Crop images in directory to face
    if args.dir0 is not None:
        print("In dir")



        """ TESTING CROPPING FEATURE"""
        counter = 0
        for imgPath in os.listdir(args.dir0):
            #imgPath = "data/nicholas_akin/img_43.jpg"        
            imgPath = args.dir0 + "/" + imgPath
            ...
            tm.stop()
            assert faces1[1] is not None, 'Cannot find a face in {}'.format(imgPath)

            # Draw results on the input image
            #visualize(img, faces1, tm.getFPS())

            #Crop image to face
            cimg = crop(img, faces1)

            # Save results if save is true
            if args.save:
                print('Results saved to data/cropped/\n')
                cv.imwrite('data/cropped/img_' + str(counter) + '.jpg', cimg)
                counter += 1
        ...
        if args.image1 is not None:
            ...
            while cv.waitKey(1) < 0:
                ...
                #saving video frame
                if (args.save == True):
                    cv.imwrite('data/frames/frame' + str(frameCounter) + '.jpg', frame)
                frameCounter += 1
